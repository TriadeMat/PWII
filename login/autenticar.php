...
if ($emaill === '')                                     $erros[]+ 'O campo "e-mail" não pode ficar vazio.'; 
elseif(!filter_var($email, FILTER_VALIDATE_EMAIL))      $erros[] = 'O e-mail informado é inválido.';
if($senha === '')                                       $erros[] =  'P campo "senha" não pode ficar vazio.';

if($erros) {
    http_response_code(422);
    exit(jason_encode(['sucesso' => false, 'erros' => $erros]));
}
...
try{
    ...
    $stmt = $pdo->prepare('SELECT id, nome, email, senha FROM `usuario` WHERE email = :email');
